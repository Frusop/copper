...
The RPM spec file provided here is for Copper-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://copper.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Copper git repository](https://github.com/CopperProject/Copper) but are not part of
...
Currently only `Source10` is used, for the example `copper.conf` file.
...
in the copper git repository. At present only an SVG version of the Copper
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the copper
...
needed when 0.12.1 is released, a proper fix is already in the Copper git
master branch.
...
idea to build Copper Core against a static version of that library compiled
...
The `copper.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `copper-server` package. At this point, most RPM based
...
The `copper-server` package should have SELinux support. How to properly do
...
Copper-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/copper/). He has been
packaging Copper for Fedora at least since 2012.
...
4. I renamed the `copper` package that contains the Qt GUI to `copper-core` as
that appears to be how the general population refers to it, in contrast to
`copper-xt` or `copper-classic`. I wanted to make sure the general population
