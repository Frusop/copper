...
###   coppercore.org and copper.org and compares them.
...
WORKINGDIR="/tmp/copper_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://copper.org"
HOST2="https://copper.org"
BASEDIR="/bin/"
VERSIONPREFIX="copper-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'copper-',
...
if [ $? -ne 0 ]; then
   echo "copper.org failed to provide signature file, but coppercore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "copper.org and coppercore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Copper Core binary release signing key installed?"
   fi
