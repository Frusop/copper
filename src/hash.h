...
#ifndef COPPER_HASH_H
#define COPPER_HASH_H
#include <iostream>
#include <chrono>
...
/** A hasher class for Copper's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Copper's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
    ...
    for (int i=0;i<23;i++) 
    {
        ...
        switch(hashSelection) {
            ...
            case 20:
                ...
                break;
            case 21:
                hash_argon2d(static_cast<void*>(&hash[i]), lenToHash, toHash, lenToHash, toHash, lenToHash, t_costs, m_costs);
                break;
            case 22:
                hash_argon2i(static_cast<void*>(&hash[i]), lenToHash, toHash, lenToHash, toHash, lenToHash, t_costs, m_costs);
                break;
        }
    }
